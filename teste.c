...
int quantNumerosIntervalo (TLista L, int numA, int numB)
{
	...
	while (aux != NULL) //enquanto 'aux' for diferentes de NULL; ou seja, enquanto existir nó na lista.
	{	
		if (aux->valor == numA ) //verificando se o elemento apontado por 'aux' é o valor 'numA'.
		{
			aux1 = aux;
			aux = aux->prox;
	...
	return cont; //retornando a quantidade de números no intervalo 'aux1 <-> aux'
}
